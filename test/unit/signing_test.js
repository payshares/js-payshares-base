...
describe('PaysharesBase#sign', function() {
  ...
  it("can sign an string properly", function() {
    ...
    let actualSig   = PaysharesBase.sign(data, secretKey).toString('hex');
...
describe('PaysharesBase#verify', function() {
  ...
  it("can verify an string properly", function() {
    let data = "hello world";
    expect(PaysharesBase.verify(data, sig, publicKey)).to.be.truthy;
    expect(PaysharesBase.verify("corrupted", sig, publicKey)).to.be.falsey;
    expect(PaysharesBase.verify(data, badSig, publicKey)).to.be.falsey;
  });
