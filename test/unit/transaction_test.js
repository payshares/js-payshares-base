...
describe('Transaction', function() {
  ...
  it("constructs Transaction object from a TransactionEnvelope", function(done) {
    let source      = new PaysharesBase.Account("GBBM6BKZPEHWYO3E3YKREDPQXMS4VK35YLNU7NFBRI26RAN7GI5POFBB", "0");
    let destination = "GDJJRRMBK4IWLEPJGIE6SXD2LP7REGZODU7WDC3I2D6MR37F4XSHBKX2";
    let asset       = PaysharesBase.Asset.native();
    let amount      = "2000";

    let input = new PaysharesBase.TransactionBuilder(source)
                .addOperation(PaysharesBase.Operation.payment({destination, asset, amount}))
                .addMemo(PaysharesBase.Memo.text('Happy birthday!'))
                .build()
                .toEnvelope()
                .toXDR('base64');


    var transaction = new PaysharesBase.Transaction(input);
    ...
    expect(transaction.memo.type).to.be.equal(PaysharesBase.MemoText);
  ...
  it("does not sign when no Network selected", function() {
    ...
    let asset       = PaysharesBase.Asset.native();
    let amount      = "2000";
    let signer      = PaysharesBase.Keypair.random();

    let tx = new PaysharesBase.TransactionBuilder(source)
                .addOperation(PaysharesBase.Operation.payment({destination, asset, amount}))
                .build();
  ...
  it("signs correctly", function() {
    ...
    let asset       = PaysharesBase.Asset.native();
    let amount      = "2000";
    let signer      = PaysharesBase.Keypair.master();
  ...
  it("accepts 0 as a valid transaction fee", function(done) {
    ...
    let input = new PaysharesBase.TransactionBuilder(source, {fee: 0})
                .addOperation(PaysharesBase.Operation.payment({destination, asset, amount}))
                .addMemo(PaysharesBase.Memo.text('Happy birthday!'))
                .build()
                .toEnvelope()
