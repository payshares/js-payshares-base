...
describe("Memo.constructor()", function() {
  it("throws error when type is invalid", function() {
    expect(() => new PaysharesBase.Memo("test")).to.throw(/Invalid memo type/);
  });
});

describe("Memo.none()", function() {
  it("converts to/from xdr object", function() {
    let memo = PaysharesBase.Memo.none().toXDRObject();
    expect(memo.value()).to.be.undefined;

    let baseMemo = PaysharesBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(PaysharesBase.MemoNone);
...
describe("Memo.text()", function() {
  ...
  it("returns a value for a correct argument", function() {
    expect(() => PaysharesBase.Memo.text("test")).to.not.throw();
    let memoUtf8 = PaysharesBase.Memo.text("三代之時")
  ...
  it("converts to/from xdr object", function() {
    let memo = PaysharesBase.Memo.text("test").toXDRObject();
    expect(memo.arm()).to.equal('text');
    expect(memo.text()).to.equal('test');
    expect(memo.value()).to.equal('test');

    let baseMemo = PaysharesBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(PaysharesBase.MemoText);
    expect(baseMemo.value).to.be.equal('test');
  });

  it("throws an error when invalid argument was passed", function() {
    expect(() => PaysharesBase.Memo.text()).to.throw(/Expects string/);
    expect(() => PaysharesBase.Memo.text({})).to.throw(/Expects string/);
    expect(() => PaysharesBase.Memo.text(10)).to.throw(/Expects string/);
    expect(() => PaysharesBase.Memo.text(Infinity)).to.throw(/Expects string/);
    expect(() => PaysharesBase.Memo.text(NaN)).to.throw(/Expects string/);
  });

  it("throws an error when string is longer than 28 bytes", function() {
    expect(() => PaysharesBase.Memo.text("12345678901234567890123456789")).to.throw(/Text should be/);
    expect(() => PaysharesBase.Memo.text("三代之時三代之時三代之時")).to.throw(/Text should be/);
  });
...
describe("Memo.id()", function() {
  it("returns a value for a correct argument", function() {
    expect(() => PaysharesBase.Memo.id("1000")).to.not.throw();
    expect(() => PaysharesBase.Memo.id("0")).to.not.throw();
  });

  it("converts to/from xdr object", function() {
    let memo = PaysharesBase.Memo.id("1000").toXDRObject();
    expect(memo.arm()).to.equal('id');
    expect(memo.id().toString()).to.equal('1000');

    let baseMemo = PaysharesBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(PaysharesBase.MemoID);
    expect(baseMemo.value).to.be.equal('1000');
  });

  it("throws an error when invalid argument was passed", function() {
    expect(() => PaysharesBase.Memo.id()).to.throw(/Expects a int64/);
    expect(() => PaysharesBase.Memo.id({})).to.throw(/Expects a int64/);
    expect(() => PaysharesBase.Memo.id(Infinity)).to.throw(/Expects a int64/);
    expect(() => PaysharesBase.Memo.id(NaN)).to.throw(/Expects a int64/);
    expect(() => PaysharesBase.Memo.id("test")).to.throw(/Expects a int64/);
  });
});
...
describe("Memo.hash() & Memo.return()", function() {
  it("hash converts to/from xdr object", function() {
    ...
    let memo = PaysharesBase.Memo.hash(buffer).toXDRObject();
    expect(memo.arm()).to.equal('hash');
    expect(memo.hash().length).to.equal(32);
    expect(memo.hash()).to.deep.equal(buffer);

    let baseMemo = PaysharesBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(PaysharesBase.MemoHash);
  ...
  it("return converts to/from xdr object", function() {
    ...
    // Testing string hash
    let memo = PaysharesBase.Memo.return(buffer.toString("hex")).toXDRObject();
    ...
    let baseMemo = PaysharesBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(PaysharesBase.MemoReturn);
    expect(Buffer.isBuffer(baseMemo.value)).to.be.true;
    expect(baseMemo.value.length).to.equal(32);
    expect(baseMemo.value.toString('hex')).to.be.equal(buffer.toString('hex'));
  });

  var methods = [PaysharesBase.Memo.hash, PaysharesBase.Memo.return];
